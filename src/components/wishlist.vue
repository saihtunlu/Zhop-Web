<template>
  <div class="row myWishlist">
    <div class="col-12 mt-3">
      <h3 class="upper flex-align-center p-3" style="margin-left:14px">
        <vs-button
          radius
          color="#000"
          class="darkBtn mr-2"
      ...
      <fade-transition class="row mx-sm-0 mt-3 sm-padding" v-if="favs.length > 0" group>
        <div class="col-lg-3 col-md-4 col-sm-6" v-for="fav in favs" :key="fav.product.title">
          <Card :product="fav.product" />
        </div>
      </fade-transition>
      <div class="flex-center" v-else>
        <div class="flex-center flex-column">
          <svg xmlns="http://www.w3.org/2000/svg" width="80%" height="328" viewBox="0 0 347 328">
            <defs>
              <filter
                id="a"
                width="173.2%"
          ...
          <p
            class="text-center text-bolder transparent_text caption"
          >Oops! No product in your wishlist :(</p>
          <vs-button
            class="mt-3"
            @click="
              $router.push({
