<template>
  <div class="row lg-80 mt-medium">
    <div class="col-lg-6" v-if="data.cart.length > 0">
      <div class="checkoutBox shadow">
        <h3 class="upper flex-align-center mb-3">
          <vs-button
            radius
            color="#fff"
            ...
            icon-pack="bx"
            icon="bx-user"
          ></vs-button>User Information
        </h3>
        <vs-input label="Name*" class="w-100 mb-3" v-model="data.userName" />
        <vs-input label="Email Address" class="w-100 mb-3" type="email*" v-model="data.email" />
        <vs-input label="Phone" class="w-100" type="number*" v-model="data.phone" />
      </div>
      <div class="checkoutBox mt-medium shadow">
        <h3 class="upper mb-3 flex-align-center">
          <vs-button
            radius
            color="#fff"
            class="sm-hide whiteBtn mr-2"
        ...
        <vs-input label="Address Line 1*" class="w-100 mb-3" v-model="data.addressLine1" />
        <vs-input label="Address Line 2 (optional)" class="w-100" v-model="data.addressLine2" />
      </div>
    </div>
    <div class="col-lg-6" v-if="data.cart.length > 0">
      <div class="checkoutBox my-sm-3 shadow">
        <h3 class="upper mb-3 flex-align-center">
          <vs-button
            radius
            color="#fff"
          ...
          ></vs-button>Your Order
        </h3>
        <div class="cartProducts mb-3">
          <div
            class="cartCart mb-3 w-100 flex-x-between"
            v-for="(product, index) in data.cart"
      ...
      <div class="checkoutBox shadow my-3">
        <p class="mb-3 upper text-bold flex-align-center">
          ...
          ></vs-button>Payment Methods
        </p>
        <vs-collapse v-if="paymentMethods.length > 0" id="paymentMethod">
          <vs-collapse-item
          ...
          >
            <div slot="header">
              <vs-radio
                v-model="data.selectedPayment"
                vs-name="radios1"
                :vs-value="payment"
                @change="changePayment()"
              >{{ payment.type }}</vs-radio>
            </div>
            {{ payment.description }}
          </vs-collapse-item>
        </vs-collapse>
        <p class="text-center" v-else>No payment methods available!</p>
        <vs-divider />
        <div class="flex-x-between">
          <p class="upper">Subtotal</p>
          <p>{{ cartTotal }} Ks</p>
        </div>
        <vs-divider />
        <div class="flex-x-between">
          <p class="upper">Total weight</p>
          <p>{{ data.totalWeight }} Kg</p>
        </div>
        <vs-divider />
        <div class="flex-x-between">
          <p class="upper">Delivery Price</p>
          <p>
            {{
            !data.shippingPrice
        ...
        <vs-divider />
        <div class="flex-x-between">
          <p class="upper">Grand Total</p>
          <p>{{ data.totalPrice }} Ks</p>
        </div>
        <vs-divider />
        <p class="mb-3">
        ...
        <div class="flex-align-center">
          <vs-checkbox v-model="agreement" class="mr-2">
            <p class="primary sm-text">I have read and agree to the website's term and conditions.*</p>
          </vs-checkbox>
        </div>
        <vs-button
          @click="startOrder"
          class="w-100 mt-medium"
          :disabled="error || !agreement || !data.selectedPayment"
        >
          Place Order
          <vs-icon icon="bx-loader-circle bx-spin" icon-pack="bx" color="#fff" v-if="loading" />
        </vs-button>
      </div>
    </div>
    <div class="col-12 flex-center" style="height:calc(100vh - 70px)" v-else>
      <div class="flex-center flex-column">
        <p
          class="transparent_text mb-5 text-center text-bolder caption"
        >No products found in your bag!</p>
        ...
        <vs-button to="/shop">Start Shopping</vs-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    this.data.addressLine1 = this.address.addressLine1;
    this.data.addressLine2 = this.address.addressLine2;
    this.changeState();
    this.calculate();
  },
  methods: {
    ...
    calculate() {
      ...
      var totalWeight = parseFloat(
        weightArray.reduce((sum, item) => sum + item, 0)
      ).toFixed(2);
      ...
      if (!getCity) {
        this.$vs.notify({
          title: "Error",
          text: `${city} is current unavailable for shipping now!`,
          color: "danger",
        });
        this.data.shippingPrice = null;
      ...
      var GrandTotal =
        parseInt(shippingPrice) + parseInt(total) + parseInt(total) * 0.05;
      this.data.shippingPrice = shippingPrice;
      this.data.totalPrice = GrandTotal;
    },
    calculatePayment(city, totalPrice) {
      var payments = [];
      const PaymentsMethods = this.payments;
      PaymentsMethods.forEach((data) => {
        if (data.restrictions) {
          let getResult = [];
          let restriction1 = data.restrictions[0];
          if (restriction1.by === "Cities") {
            if (restriction1.condition === "Is") {
              let check = restriction1.cities.filter((data) => {
                return data.value === city;
              });
              if (check.length > 0) {
                let resultArray = {};
          ...
          if (restriction1.by === "Shipping") {
            let cities = restriction1.shipping.cities;
            if (restriction1.condition === "Is") {
              let check = cities.filter((data) => {
                return data.value === city;
              });
              if (check.length > 0) {
                let resultArray = {};
          ...
          if (data.restrictions[1]) {
            let restriction2 = data.restrictions[1];
            if (restriction2.by === "Cities") {
              if (restriction2.condition === "Is") {
                let check = restriction2.cities.filter((data) => {
                  return data.label === city;
                });
                if (check.length > 0) {
                  let resultArray = {};
            ...
            if (restriction2.by === "Shipping") {
              let cities = restriction2.shipping.cities;
              if (restriction2.condition === "Is") {
                let check = cities.filter((data) => {
                  return data.value === city;
                });
    ...
    changePayment() {
      console.log(this.data.selectedPayment);
    },
    startOrder() {
      this.loading = true;
      this.$axios
        .post(`/startOrder`, { data: this.data })
        .then((response) => {
          this.$vs.notify({
            title: "Success",
          ...
          var cart = [];
          this.$store.dispatch("addToCart", { cart });
          var orders = [...this.$store.state.orders, response.data];
          this.$store.commit("setUserOrders", orders);
          this.loading = false;
          this.$router.push({
            name: "OrderComplete",
...
.cartProducts .cartCart img.pointer {
  object-fit: cover;
  object-position: center;
}
.whiteBtn {
  color: rgb(var(--vs-dark)) !important;
  box-shadow: none !important;
  background: rgb(var(--vs-gray)) !important;
  border-radius: 15px !important;
}
div#paymentMethod .vs-collapse-item {
  transition: 0.3s all;
  background: rgb(var(--vs-gray));
  border-radius: 15px;
  margin-bottom: 10px;
}
div#paymentMethod .con-vs-radio {
