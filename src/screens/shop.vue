<template>
  <div class="shop-page row">
    <div class="col-12 p-0">
      <div
        id="shop_banner"
        class="div-bg light-shadow flex-center"
        :style="`background:url('/${banner.src}')`"
      >
        <div class="flex-center flex-column">
          <p class="transparent_text mb-3 text-center text-bolder header upper">{{ banner.title }}</p>
          <p
            class="text-center mb-base sm-hide"
            style="color:rgb(0,0,0,0.5); font-size:18px;"
          >{{ banner.description }}</p>
          <div
            class="searchDiv mt-2 flex-between-center sm-hide"
            style="box-shadow:rgba(0, 0, 0, 0.05) 0px 21px 36px"
          >
            <p>{{ category ? category.label : "All" }}</p>
            <vs-input
              @click="backupData = products"
              class="w-100"
            ...
            >
              <vs-icon icon="bx-search" icon-pack="bx" class="mr-2" />
              <span>Search</span>
            </a>
          </div>
        </div>
    ...
    <vs-divider class="mb-sm-2 mt-0" id="_to" />
    <div class="col-12 flex-align-center" style="overflow-x:auto">
      <a
        class="navbar-item mr-lg-3 mr-md-3 mr-sm-2"
        :class="{ 'is-active-item': cid == null }"
        @click="
          $router.push({
            name: 'Shop',
          })
        "
      >
        <vs-icon icon="bx-grid-alt" class="mr-2" icon-pack="bx" />All
      </a>
      <a
        class="navbar-item mr-lg-3 mr-md-3 mr-sm-2"
        :class="{ 'is-active-item': cid == category.id }"
    ...
    <vs-divider class="mb-0 mt-sm-0" />
    <div class="col-lg-3 pt-3" style="border-right:1px solid rgb(var(--vs-gray2))">
      <div class="categoriesList">
        <vs-collapse>
          <vs-collapse-item class="category2" icon-pack="bx" icon-arrow="bx-chevron-down">
            <div slot="header">
              <div class="flex-align-center py-2">
                <vs-icon icon="bx-slider-alt" icon-pack="bx" class="mr-2" />
                <p>Filter All Products By Price</p>
              </div>
            </div>
            <vs-slider
              @change="changePrice"
              ...
              text-fixed="%"
            />
            <p class="text-center">{{ minPrice }}Ks - {{ maxPrice }}Ks</p>
          </vs-collapse-item>
        </vs-collapse>
        <vs-divider class="my-0" />
        <vs-collapse v-if="category">
          <vs-collapse-item
            v-for="category2 in category.children"
            :key="category2.label"
            class="category2"
            icon-pack="bx"
            icon-arrow="bx-chevron-down"
          >
            <div slot="header" class="flex-align-center">
              <img
                :src="`https://zhop.admin.saihtunlu.me${category2.image}`"
                class="mr-3 categoryImage"
              />
              {{ category2.label }}
            </div>
            <a
              ...
              @click="
                () => {
                  selectedCategory_id = category3.id;
                  filterProductsByCategory();
                }
              "
              :class="{
                'active-category': selectedCategory_id === category3.id,
              }"
              v-for="category3 in category2.children"
              class="category3 flex-x-between pb-2 mt-2"
              :key="category3.label"
            >
              <div class="flex-align-center">
                <img
                  :src="`https://zhop.admin.saihtunlu.me${category3.image}`"
                  alt
                  class="mr-3 categoryImage"
                />
                <p class="text-bold;font-size:1.2rem !important;">{{ category3.label }}</p>
              </div>
              <div class="dot" v-if="selectedCategory_id === category3.id"></div>
            </a>
          </vs-collapse-item>
        </vs-collapse>
      </div>
    ...
    <div
      class="col-lg-9 pt-3"
      id="productsList"
      style="background: rgb(var(--vs-gray));min-height:100vh;"
    >
      <fade-transition class="row sm-padding mx-sm-0" v-if="products.length !== 0" group>
        <div
      ...
      <div class="flex-center" style="height:100%" v-if="products.length === 0">
        <div class="flex-center flex-column">
          <p
            class="mb-5 text-center text-bolder transparent_text caption"
          >Oops! We couldn’t find results for you :(</p>
      ...
      <div class="flex-center" v-else>
        <vs-button color="primary" @click="loadMore()" type="flat">
          Load More
          <i
            v-if="loading"
            class="bx flex-center ml-2 bx-loader-circle bx-spin"
...
<script>
import banners from "../constants/banner.js";
import Card from "../components/productCart";
import { mapGetters, mapState } from "vuex";
const defaultBanner = {
  src: "image/all.png",
  title: "Our store has everything you love",
  description: "We have everything in our store for you! ",
};
export default {
  data() {
    return {
      ...
      query: null,
      products: [],
      number: 12, //number of products for show
      category: null,
      target: null,
      cid: null,
      backupData: [],
  ...
  computed: {
    ...
    ...mapGetters({
      ...
      discount: "discount",
      priceRangeAll: "priceRange",
      priceRangeByCategory: "priceRangeByCategory",
      filterPriceByCategory: "filterPriceByCategory",
      filterPriceByAll: "filterPriceByAll",
  ...
  mounted() {
    this.minPrice = this.calculate(0);
    this.maxPrice = this.calculate(100);
  },
  methods: {
    check() {
      ...
      if (target === "category") {
        ...
        this.cid = cid;
        this.priceRange = this.priceRangeByCategory(parseInt(cid));
        this.category = this.selectedCategory(parseInt(cid));
        this.sideBarLabel = this.category.label;
        this.selectedCategory_id = this.category.children[0].children[0].id;
        this.banner = banners.filter((data) => {
          return data.type === this.category.label;
        })[0];
        this.filterProductsByCategory();
      } else {
        this.getProducts(target);
      }
    },
    search(query) {
      if (query === "") {
        ...
        if (this.target === "category") {
          ...
          data.query = query;
          this.products = this.searchProduct(data);
        }
        if (!this.target) {
          this.products = this.searchAllProduct(query);
        }
      }
    },
    filterProductsByCategory() {
      var data = {};
      data.id = this.selectedCategory_id;
      data.number = this.number;
      this.products = this.filterProduct(data);
      this.loading = false;
    },
    changePrice(values) {
      this.minPrice = this.calculate(values[0]);
      this.maxPrice = this.calculate(values[1]);
      this.filterPrice(this.minPrice, this.maxPrice);
    },
    calculate(value) {
      var min = this.priceRange[0];
      var max = this.priceRange[1];
    ...
    filterPrice(min, max) {
      ...
      if (this.target === "category") {
        data.id = this.category.id;
        this.products = this.filterPriceByCategory(data);
      } else {
        this.products = this.filterPriceByAll(data);
      }
    },
    getProducts(target) {
      if (target === "featured") {
        this.target = target;
        this.products = this.featured(this.number);
        this.sideBarLabel = "Featured Products";
      }
      if (target === "newArrival") {
        this.target = target;
        this.products = this.newArrival(this.number);
        this.sideBarLabel = "New Arrivals";
      }
      if (target === "discount") {
        this.target = target;
        this.products = this.discount(this.number);
        this.sideBarLabel = "Discount Products";
      }
      if (target === "sellProduct") {
        this.target = target;
        this.products = this.sellProduct(this.number);
        this.sideBarLabel = "Top Sell Products";
      }
      if (target === "brand") {
        var data = {};
        data.id = parseInt(this.$route.query.bid);
        var brand = this.$route.query.bname;
        data.number = this.number;
        this.target = target;
        this.products = this.getBrandProduct(data);
        this.sideBarLabel = `Products By ${brand}`;
      }
      if (target === "related") {
        var id = parseInt(this.$route.query.pid);
        var params = {};
        params.product = this.getProduct(id);
        params.number = this.number;
        this.target = target;
        this.products = this.getRelatedProduct(params);
      }
      if (typeof target === "undefined") {
        this.products = this.getAllProducts(this.number);
        this.sideBarLabel = "All Products";
      }
      this.priceRange = this.priceRangeAll;
      this.loading = false;
    },
    loadMore() {
      this.loading = true;
...
<style>
.vs-collapse-item.category2.open-item {
  background: rgb(var(--vs-gray)) !important;
}
.vs-collapse-item.category2.open-item header.vs-collapse-item--header {
  background: rgb(var(--vs-primary));
  border-top-right-radius: 15px !important;
  border-top-left-radius: 15px !important;
}
.vs-collapse-item.category2 {
  border-radius: 15px !important;
  overflow: hidden;
  transition: 0.3s all ease-in;
}
.searchDiv .vs-input--placeholder {
  color: rgb(var(--vs-dark), 0.7) !important;
...
div#category-slider button.agile__nav-button {
  border: 0px;
  border-radius: 10rem;
  background: rgb(var(--vs-primary));
  color: #fff;
...
a.navbar-item.is-active-item {
  background: rgb(var(--vs-gray));
  color: rgb(var(--vs-primary));
}
.searchDiv .vs-con-input > input.vs-inputx {
  border: none !important;
  box-shadow: none !important;
}
.searchDiv input.vs-inputx.vs-input--input {
...
div#category-slider button.agile__nav-button:hover {
  box-shadow: 0px 8px 25px -4px rgb(var(--vs-primary), 0.7) !important;
}
div#category-slider .agile__list {
  overflow: visible !important;
}
.vs-collapse-item.category2.open-item > header > div,
.vs-collapse-item.category2.open-item
  span.icon-header.vs-collapse-item--icon-header {
  color: #fff;
}
.category3,
.categories {
  color: rgb(var(--vs-dark));
  border-bottom: 1px solid rgb(var(--vs-gray));
  transition: 0.3s all;
}

.category3.active-category,
...
@media (min-width: 992px) {
  ...
  div#category-slider .agile__actions {
    width: 100% !important;
  }
  .categoriesList {
    /* padding: 15px; */
